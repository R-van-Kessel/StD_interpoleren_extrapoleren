...
const SUBTITLE_TEXT = 'Los alle sommen op (interpoleren/extrapoleren) om de draak op te roepen!';
...
function generateInterpolationQuestion() {
  // Willekeurige stapgrootte (1 t/m 5, hele getallen)
  let dx = floor(random(1, 6));
  
  // Startpunt x0 (5 t/m 20)
  let x0 = floor(random(5, 21));
  let x1, x2;
  ...
  if (isExtrapolation) {
    ...
    x1 = x0 + dx;
    x2 = x0 + 2 * dx;  // we verbergen x2 of een y buiten bereik
  } else {
  ...
  // a: 0.5, 1, 1.5, 2, 2.5, 3 (altijd netjes)
  let aOptions = [0.5, 1, 1.5, 2, 2.5, 3, -0.5, -1, -1.5, -2];
  let a = aOptions[floor(random(aOptions.length))];
  let b = floor(random(0, 21));   // b: 0 t/m 20
...
function showInfo() {
    ...
    popup.innerHTML = `
        ...
        <p style="color: #0E0E0E; line-height: 1.2;">
            <strong>Doel:<br></strong> Los alle 10 tabellen correct op met behulp van interpoleren of extrapoleren en speel de Dragon game!<br><br>
